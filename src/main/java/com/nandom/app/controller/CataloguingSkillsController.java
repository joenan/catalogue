 ...
 */
@Controller
public class CataloguingSkillsController {
    ...
    public String examStatusAlert = "<div id=\"success-alert\" class=\"alert alert-danger\" data-auto-dismiss=\"3000\">\n"
     ...
     public String examCompleted = "<div id=\"success-alert\" class=\"alert alert-danger\" data-auto-dismiss=\"3000\">\n"
            + "Cataloguing Skills Assessment Completed\n"
            + "</div>";

    @GetMapping("/cataloguingskills")
    public String getIndexingSkills(ModelMap model, HttpServletRequest request) {

        HttpSession session = request.getSession(false);
        Userprofile user = (Userprofile) session.getAttribute("user");

        List<Cataloguingskills> list = service.getCataloguingSkillsService().findByStatus(user, AssessmentStatus.WRITTEN);
        if (list != null && !list.isEmpty()) {
            ...
            return "category/cataloguingskills";
        }

    }

    @PostMapping("/cataloguingskills")
    public String getProfessionalSkillsQuestions(
            ...
            @RequestParam(value = "question-36-answers", required = false) String q36,
            ModelMap model
    ) {
